  ...
  addBtn.addEventListener('click', () => {
    ...
    .then(data => {
      if (data.success) {
        ...
        li.dataset.libraryId = data.library.lib_code;
        li.innerHTML = `
          ${data.library.name} - ${data.library.address}
          <button class="removeFavoriteBtn" data-library-id="${data.library.lib_code}">삭제</button>
        `;
  ...
  favoritesList.addEventListener('click', e => {
    if (e.target.classList.contains('removeFavoriteBtn')) {
      ...
      fetch('/mypage/favorites/remove', {
        ...
        body: JSON.stringify({ libCode: libraryId })
      })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
